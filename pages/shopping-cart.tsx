...
const shoppingCart: NextPage = () => {
  ...
  const handleSubmit = async (event: any) => {
    ...
    const producIds: string[] = [];
    products.map((product) => producIds.push(product._id));
    const sell = await sellProduct(api, token, producIds);
    if (!sell) {
...
const PaymentShipping = styled.p`
  font-size: 1rem;
  font-weight: 300;
  color: ${({ theme }) => theme.colors.secondary};
  margin: 0;
